...
GameObject ObjectConstructor::Construct(GameSession *gameSession) const {
    LOG_DEBUG( this->name + "  생성 시작");

    // 1. 세션 자체 널 체크
    if (!gameSession || !gameSession->objectManager) {
        LOG_ERROR("[Fatal Error] gameSession or objectManager is null!");
    ...
    if (!obj) {
        LOG_ERROR("[Fatal Error] CreateGameObject() returned null!");
        return obj;
    }

    LOG_DEBUG("GameObject 기본 할당 완료.");
    ...
    if (!obj->gameSession) {
        LOG_ERROR("[Fatal Error] obj->gameSession is null! 초기화 누락됨!");
        // 임시 조치로 강제로 넣어줌 (원래는 CreateGameObject 안에서 해줘야 함)
        obj->gameSession = gameSession;
    }

    for (auto& v: this->components) {
        LOG_DEBUG("부착 시도 중: " + v.ComponentName);
        v.ConstructAndAttachTo(obj); // 이 안에서 터진다면 1번이나 2번 원인
        LOG_DEBUG( v.ComponentName + "  컴포넌트 부착 완료");
    }

    LOG_DEBUG( this->name + "  생성 완전 종료");
    return obj;
}
