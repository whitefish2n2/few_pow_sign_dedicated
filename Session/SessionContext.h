...
inline thread_local GameSession* gameSessionInstance;
/// 스레드 내부 전역 게임오브젝트 매니저 인스턴스(해당 스레드 GameSession::Start()에서 초기화)
inline thread_local GameObjectManager *gameObjectManagerInstance;
/// 스레드 내부 전역 컴포넌트 매니저 인스턴스(해당 스레드 GameSession::Start()에서 초기화)
inline thread_local ComponentManager* componentManagerInstance;
